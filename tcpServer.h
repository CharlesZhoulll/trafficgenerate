...
/* Get current time. */
void get_now(struct timeval *time)
{
...
double time_to_ms(struct timeval *tstart, struct timeval *tfinish)
{
    double t;
    t = (tfinish->tv_sec - tstart->tv_sec) * 1e3 + (tfinish->tv_usec - tstart->tv_usec) / 1e3;
    return t;
}
